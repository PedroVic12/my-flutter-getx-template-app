...
class _KanbanProAppState extends State<KanbanProApp> {
  ...
  Widget _renderCurrentScreen() {
    switch (_currentScreen) {
      ...
      case 'kanban':
        return KanbanScreen(repository: _repo, refreshParent: _loadData);
      case 'table':
        return Center(
          child: Text('Table Screen', style: TextStyle(color: Colors.black)),
        );
      case 'files':
        return Center(
          child: Text('Files Screen', style: TextStyle(color: Colors.black)),
        );
      case 'pomodoro':
        return Center(
          child: Text('Pomodoro Screen', style: TextStyle(color: Colors.black)),
        );
      default:
        return KanbanScreen(repository: _repo, refreshParent: _loadData);
    }
  ...
  Widget build(BuildContext context) {
    // Para telas maiores, exibe o sidebar fixo.
    // Em telas menores, ele é controlado por um Drawer.
    bool isLargeScreen = MediaQuery.of(context).size.width > 800;

    return Scaffold(
      appBar: isLargeScreen ? null : AppBar(title: const Text('Kanban Pro')),
      drawer: isLargeScreen
          ? null
          : AppDrawer(
              currentScreen: _currentScreen,
              onScreenSelected: _setCurrentScreen,
            ),
      body: _isLoading
          ...
          : Row(
              children: [
                ...
                Expanded(child: _renderCurrentScreen()),
              ],
            ),
    );
  }
...
class AppDrawer extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildDrawerItem(context, 'kanban', Icons.view_kanban, 'Kanban'),
          _buildDrawerItem(context, 'table', Icons.table_chart, 'Tabelas'),
          _buildDrawerItem(context, 'files', Icons.folder, 'Arquivos'),
          _buildDrawerItem(context, 'pomodoro', Icons.timer, 'Pomodoro'),
          const Divider(),
          _buildDrawerItem(context, 'export', Icons.download, 'Backup Excel'),
          _buildDrawerItem(context, 'import', Icons.upload, 'Importar Excel'),
          _buildDrawerItem(context, 'sync', Icons.sync, 'Sincronizar'),
        ],
      ),
    );
  }
  ...
  Widget _buildDrawerItem(
    ...
    final isSelected = currentScreen == id;
    return Material(
